...
class EnvV6Controller(Controller):
    """
    Version 6 Environment Controller
    ...
    def _get_state(self):
        ...
        state[0] = sc.current_to_draw  # to draw(dim=1),        #0
        state[sc.current_color.value] = 1  # color(dim=4),      #1 - #4
        state[sc.current_value + 6] = 1  # value(dim=11),       #5 - #15
        state[sc.current_type.value + 16] = 1  # type(dim=6), # #16 - #21

        # flow state(dim=2): clockwise(dim=2)
        state[int(self.flow_controller.clockwise) + 22] = 1  # #22 - #23
    ...
    def step(self, action):
        ...
        playables = ep.get_playable(sc.current_color, sc.current_value, sc.current_type, sc.current_to_draw)
        play = None
        reward = np.full((self.action_space_dim,), -1)
        for i, card in playables:
        ...
        if play is not None:
            ...
            if done:
                card = play[1]
                reward[self.action_invmap[card.short_name]] += self.players[1].num_cards
        else:
            prev_nc = ep.num_cards
            self.apply_penalty(ep)
            done = False
            reward[idx] = prev_nc - ep.num_cards
