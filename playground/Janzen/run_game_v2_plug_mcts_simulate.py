...
if __name__ == "__main__":
    ...
    parser.add_argument('--mcts_iter', '-n', type=int, required=True)
    kwargs = dict(parser.parse_args()._get_kwargs())
    mcts_iter = kwargs.pop('mcts_iter')
    # =======
    # players
    # =======
    mcts_get_play = MCTSGetPlayPolicy(mcts_iter=mcts_iter)
    target_player_tup = (
        PlayerType.POLICY,
        "MCTS_{}".format(mcts_iter),
        dict(get_play=mcts_get_play)
    )
